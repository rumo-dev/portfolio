...
<Card title="Post Effect Manager" icon="rocket">
  12種類のポストエフェクトを一本のパイプラインとして統括する管理システム。各エフェクトを疎結合に連結する設計を解説します。
  <LinkCard title="Post Effect Managerの実装技術を見る" href="./post-effect-manager/" />
</Card>

<Card title="Sky" icon="moon">
  レイマーチングによる大気散乱シミュレーションと、雲・星・オーロラをプロシージャルに合成するスカイレンダリング。
  <LinkCard title="Skyの実装技術を見る" href="./sky/" />
</Card>

<Card title="Fog System" icon="seti:image">
  距離・指数・高さ・ボリュームの4方式を多段合成するフォグシステム。Froxelベースのボリュームフォグも実装。
  <LinkCard title="Fog Systemの実装技術を見る" href="./fog/" />
</Card>

<Card title="Depth of Field" icon="magnifier">
  AMD GDC 2017 Fast Filter SpreadingをCompute Shaderで実装した、真の散乱方式による被写界深度。
  <LinkCard title="Depth of Fieldの実装技術を見る" href="./dof/" />
</Card>

<Card title="Exposure" icon="seti:image">
  T値・シャッタースピード・ISOから物理的なEV100を算出する、カメラ露出のシミュレーション。
  <LinkCard title="Exposureの実装技術を見る" href="./exposure/" />
</Card>

<Card title="Chromatic Aberration" icon="approve-check">
  波長ごとの屈折率の違いによるRGBチャンネルのズレを再現する、物理ベース連動の色収差。
  <LinkCard title="Chromatic Aberrationの実装技術を見る" href="./chromatic-aberration/" />
</Card>

<Card title="Lens Distortion" icon="seti:lock">
  ブラウン–コンラディモデルのk1項による樽型・糸巻き型歪みを再現するレンズ歪曲。
  <LinkCard title="Lens Distortionの実装技術を見る" href="./lens-distortion/" />
</Card>

<Card title="Vignetting" icon="seti:image">
  cos^4則による物理モデルとアーティストカーブの2モードを切り替え可能な周辺減光。
  <LinkCard title="Vignettingの実装技術を見る" href="./vignetting/" />
</Card>
